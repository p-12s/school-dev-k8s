...
## 0. Документация
```
kubectl --kubeconfig ~/.kube/config-mcs.slurm.io explain pod
kubectl --kubeconfig ~/.kube/config-mcs.slurm.io explain pod.spec
kubectl --kubeconfig ~/.kube/config-mcs.slurm.io explain pod.spec.containers <- черед точку путешествие по объектам документации
```
